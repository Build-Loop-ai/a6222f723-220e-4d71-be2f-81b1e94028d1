...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // This function sends through the operator's Resend account, so it must
    // never be callable anonymously (spam/phishing vector). Internal callers
    // use the service role key; users must be a member of the org they email for.
